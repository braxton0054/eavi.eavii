...
export default function LecturerCalendar() {
  ...
  useEffect(() => {
    ...
    const checkAuth = async () => {
      ...
      const userCampus = session.user?.user_metadata?.campus || localStorage.getItem('lecturerCampus');
  ...
  if (loading) {
    return (
      <div className="min-h-screen w-full bg-gradient-to-br from-blue-950 via-blue-900 to-indigo-950 flex items-center justify-center">
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-blue-950 via-blue-900 to-indigo-950">
      <div className="relative z-10 w-full">
        <div className="bg-white/10 backdrop-blur-md border-b border-white/20">
          <div className="max-w-7xl mx-auto px-4 md:px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Link href="/lecturer/dashboard" className="text-blue-200 hover:text-white transition-colors">
              ...
              <div>
                ...
                <p className="text-blue-200 text-sm">{getCampusName(campus)}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-8">
          <div className="mb-6">
            ...
            <p className="text-blue-200">View upcoming term dates, CAT periods, and exam schedules</p>
          </div>

          <div className="grid grid-cols-1 gap-6">
            {calendars.length === 0 ? (
              <div className="bg-white/10 backdrop-blur-md rounded-xl p-8 border border-white/20 text-center">
                <p className="text-blue-200">No academic calendar entries available at this time.</p>
              </div>
            ) : (
                ...
                <div key={calendar.id} className="bg-white/10 backdrop-blur-md rounded-xl p-6 border border-white/20">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div className={`bg-white/5 rounded-lg p-4 border ${isDateUpcoming(calendar.cat_opening_date) || isDateUpcoming(calendar.cat_closing_date) ? 'border-yellow-500' : ''}`}>
                      <p className="text-blue-300 text-sm mb-1">CAT Period</p>
                    ...
                    <div className={`bg-white/5 rounded-lg p-4 border ${isDateUpcoming(calendar.end_term_exam_date) ? 'border-red-500' : ''}`}>
                      <p className="text-blue-300 text-sm mb-1">End Term Exam</p>
                    ...
                    <div className="bg-white/5 rounded-lg p-4">
                      <p className="text-blue-300 text-sm mb-1">Mock Exam</p>

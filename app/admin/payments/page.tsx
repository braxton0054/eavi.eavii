...
const FEE_TYPES = [
  ...
  { id: 'registration', label: 'Registration Fee', icon: '📋', amount: 0 },
  { id: 'library', label: 'Library Fee', icon: '📚', amount: 0 },
  { id: 'lab', label: 'Lab Fee', icon: '⚗️', amount: 0 },
...
export default function PaymentsPage() {
  ...
  type PaymentFormData = {
    application_id: string;
    payment_type: 'tuition' | 'practical' | 'exam' | 'registration' | 'library' | 'lab' | 'other';
    amount: number;
  ...
  useEffect(() => {
    ...
    checkAuth();
  }, [supabase, router, loadPayments, loadApplications]);
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div>
                    ...
                    <select value={formData.application_id} onChange={(e) => handleApplicationChange(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent text-base" required>
                      ...
                      {applications.map((app) => <option key={app.id} value={app.id}>{app.full_name} - {app.admission_number}</option>)}
                    </select>
                  </div>
                    ...
                    <div className="bg-gradient-to-r from-purple-50 to-indigo-50 border border-purple-100 rounded-xl p-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-gray-500">Course: {selectedApplication.course_id} | Semester {selectedApplication.current_semester}</div>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-4 gap-3">
                      {FEE_TYPES.map((fee) => (
                        <button key={fee.id} type="button" onClick={() => setFormData({ ...formData, payment_type: fee.id as 'tuition' | 'practical' | 'exam' | 'registration' | 'library' | 'lab' | 'other' })}
                  ...
                  <div className="flex gap-3 pt-4 border-t border-gray-200">
                    <button type="button" onClick={() => { setShowForm(false); setShowReceipt(false); setSelectedApplication(null); setReceiptNumber(''); }}
